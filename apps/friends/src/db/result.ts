...
// function composes purely via `.andThen()`/`.map()`/`safeTry()`/etc. from
// there on, with no bare `await` a DB hiccup could turn into an unhandled
// rejection (Hono's default error handler would still catch it, but as an
// undocumented, unstructured 500 — `query()` makes that failure a value
// instead, so callers decide explicitly what to do with it).
