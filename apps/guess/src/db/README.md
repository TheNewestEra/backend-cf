...
Each `GameDO` instance's constructor sets up `this.db` (see `./client.ts`)
and then calls `migrate()` (see `guess.model.ts`) inside
...
deliberately **not** softened to `CREATE TABLE IF NOT EXISTS`. Any `GameDO`
...
deliberate, accepted choice (see `guess.model.ts`'s `migrate()` doc
comment for the same note), not an oversight — there's no baseline/backfill
shim protecting already-provisioned instances.
...
`guess.model.ts`'s `this.db.select()/.insert()/.update()/.delete()` chains
...
1. Edit `schema.ts`.
2. `cd apps/guess && npx drizzle-kit generate` — emits a new incremental
...
3. Edit `./migrations.ts` by hand: add the new file's import and its
   ...
   existing `m0000`.
4. Deploy as usual — the next time each `GameDO` instance is touched, its
