...
export const CatalogStatusSchema = z.nativeEnum(CatalogStatus).openapi("CatalogStatus");

export const PlayStatusSchema = z.nativeEnum(PlayStatus).openapi("PlayStatus");

export const CatalogSortSchema = z.nativeEnum(CatalogSort).openapi("CatalogSort");
...
export const CatalogEntrySchema = z
    .object({
        ...
        averageRating: z.number().nullable().openapi({
            description:
                "Mean of every 1-5 star rating across this entry's whole replay chain, rounded to the nearest half star; null until it has at least one rating",
        }),
        ratingCount: z.number().int().nonnegative(),
        createdAt: z.number().int().positive(),
        creator: CatalogCreatorSchema.nullable().openapi({
            description: "Null only for entries that predate creator tracking",
        }),
        replayOf: z.string().nullable().openapi({
            description:
                "Catalog id this entry was replayed or regenerated from, one hop back; null if it wasn't created from another entry",
        }),
        replayKind: ReplayKindSchema.nullable().openapi({
            description:
                "Whether `replayOf` names a same-image replay or a freshly-regenerated source; null when `replayOf` is null",
        }),
        replayCount: z
            .number()
            .int()
